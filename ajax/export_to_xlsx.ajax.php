<?php
	...
	set_time_limit(0);
	$table_content = $_REQUEST['body'];
	// $what_it_is = trim($_REQUEST['header']);
	$what_it_is = 'report';
	$send_mail = isset($_REQUEST['send_mail']) ? $_REQUEST['send_mail'] : false;;
	$heading = '<h1 class="c">'.$what_it_is.'</h1>';
	$username = 'Shushanth';            			// user's name
	$usermail = 'shushant@raycharge.com';         // user's emailid
	$usercompany = 'Shushant';    				// user's company
	$htmltable = '<table class="table"><tr><th colspan=18>'.$heading.'</th></tr>'.$table_content.'</table>';
	$debug = false;
	$limit = 12;
	...
	require_once("../includes/xlsx/PHPExcel.php");
		  ...
		  if(is_object($ths)) {
			if($ths->length > 0) {
			  ...
			  for($x=0;$x<=$nodes;$x++) {
				...
				$headrows[$h]['bold'][] = true;//findBoldText(innerHTML($ths->item($x)));
